...
{
    class CDataGridViewTSWater : CExDataGridView
    {
        #region 静态常量
        ...
        private ITSWater m_proxyTSWater; //雨量表的操作接口
        private string m_strStaionId;    //查询的车站ID
        private DateTime m_dateTimeStart;   //查询的起点日期
        private DateTime m_dateTimeEnd;   //查询的起点日期


        public CDataGridViewTSWater()
            : base()
        {
            // 设定标题栏,默认有个隐藏列,默认非编辑模式
            this.Header = new string[]
            {
                CS_StationID,CS_StationName,CS_TimeCollected, CS_WaterStage,CS_TimeReceived, CS_ChannelType, CS_MsgType
            };
        ...
        public bool SetFilter(string strStationId, DateTime timeStart, DateTime timeEnd, bool TimeSelect)
        {
            ...
            m_proxyTSWater.SetFilter(strStationId, timeStart, timeEnd, TimeSelect);
            if (false)
            {
                // 查询失败
            ...
            else
            {
               ...
               // base.TotalPageCount = m_proxyTSWater.GetPageCount();
               // base.TotalRowCount = m_proxyTSWater.GetRowCount();
                SetWaters(m_proxyTSWater.GetPageData(1));
                return true;
            }
